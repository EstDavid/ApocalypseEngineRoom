...
function MyChars({ userID, client }) {
  ...
  const deleteChar = (id) => {
    client.delete(`characters/${id}`, { withCredentials: true }).then(
      client.get(`characters`, { withCredentials: true }).then((response) => {
        setCharList(response.data);
      })
    );
      ...
      <div className="filterDiv">
        ...
        <Select
          ...
          options={[
            {
              label: 'Systems',
              options: [...new Set(charList.map((c) => c.system))].map((s) => {
                return { value: s, label: s };
              })
            },
            {
              label: 'Playbooks',
              options: [...new Set(charList.map((c) => c.playbook))].map(
                (p) => {
        ...
        {charList
          .filter((c) => {
            return (
              ...
              activeFilters.some(
                (f) => f.value == c.playbook || f.value == c.system
              )
            );
          })
          .map((c, i) => (
